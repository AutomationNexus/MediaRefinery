...
// Catalog returned by the mocked /api/models/catalog.
const CATALOG = {
  models: [
    {
      id: "demo-model",
...
function setupFetchMock() {
  ...
  const handler = async (url: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (u.endsWith("/api/setup/bootstrap") && method === "GET") {
      return new Response(
        JSON.stringify({
    ...
    if (u.endsWith("/api/setup/bootstrap") && method === "POST") {
      bootstrapReady = true;
      return new Response(
    ...
    if (u.endsWith("/api/models/catalog")) {
    ...
    if (u.endsWith("/api/me/api-key") && method === "GET") {
      const api_keys =
        apiKeyCount === 0
          ? []
          : [{ id: 1, label: "primary", created_at: "now" }];
      return new Response(JSON.stringify({ api_keys, required_for_scans: true }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      });
    }
    if (u.endsWith("/api/me/api-key") && method === "POST") {
      const body = JSON.parse(String(init?.body ?? "{}"));
      if (!body.validate_api_key || body.api_key !== "valid-api-key") {
        return new Response(JSON.stringify({ detail: "api_key_invalid" }), {
          status: 400,
          headers: { "Content-Type": "application/json" },
        });
      }
      apiKeyCount += 1;
      return new Response(JSON.stringify({ id: 1, label: body.label }), {
        status: 201,
        headers: { "Content-Type": "application/json" },
      });
    }
  ...
  it("posts /setup/bootstrap with credentials and CSRF header", async () => {
    ...
    await waitFor(() => {
      const post = calls.find(
        (c) =>
          c.url.endsWith("/api/setup/bootstrap") &&
...
describe("Wizard — install phase (steps 3-8 happy path)", () => {
  it("walks catalog → license → install → api key → scan → done", async () => {
    ...
    // Step 6 — API key.
    await screen.findByRole("heading", { name: /Connect Immich API key/i });
    await userEvent.type(
      screen.getByLabelText(/^Immich API key$/i),
      "valid-api-key",
    );
    await userEvent.click(
      screen.getByRole("button", { name: /Validate and save/i }),
    );
    await screen.findByText(/API key saved/i);
    await userEvent.click(screen.getByRole("button", { name: /Continue/i }));
