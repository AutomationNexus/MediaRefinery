...
export default function Wizard({
  ...
  const initialInstallIndex = hasModel ? (hasApiKey ? 4 : 3) : 0;
  const [stepIndex, setStepIndex] = useState(
    phase === "setup" ? 0 : initialInstallIndex,
  );
  const total = phase === "setup" ? 2 : 6;
...
function InstallSteps({
  ...
  useEffect(() => {
    if (step === "catalog" && models === null) {
      getCatalog()
        .then((m) => setModels(m.filter(isClassifierCatalogModel)))
  ...
  const nextAfterInstall = () => setStepIndex((i) => (hasApiKey ? i + 2 : i + 1));
      ...
      <Section heading={t("wizard.install.title")}>
        ...
        <Footer>
          ...
          {!installed ? (
            ...
            <button type="button" onClick={nextAfterInstall} className={primaryBtn}>
              {t("wizard.next")}
            </button>
          )}
        </Footer>
  ...
  if (step === "api-key") {
    return (
      <Section heading={t("wizard.api_key.title")}>
        <p className="text-sm">{t("wizard.api_key.body")}</p>
        {apiKeySaved ? (
          <p
            role="status"
            aria-live="polite"
          ...
          >
            {t("wizard.api_key.saved")}
          </p>
        ) : (
          <label className="mt-4 block text-sm">
            <span className="font-medium">{t("wizard.api_key.label")}</span>
            <input
              type="password"
              autoComplete="off"
              value={apiKey}
              ...
              disabled={apiKeyBusy}
              className="mt-1 w-full rounded border border-slate-300 px-3 py-2 font-mono text-sm dark:border-slate-600 dark:bg-slate-900"
            />
          </label>
        )}
        {apiKeyError ? (
        ...
        <Footer>
          ...
          {!apiKeySaved ? (
            <button
              type="button"
              disabled={apiKeyBusy || apiKey.trim().length === 0}
              onClick={async () => {
                setApiKeyBusy(true);
                setApiKeyError(null);
                try {
                  await storeApiKey(apiKey.trim(), "primary", true);
                  setApiKey("");
                  setApiKeySaved(true);
                } catch {
                  setApiKeyError(t("wizard.api_key.error"));
                } finally {
                  setApiKeyBusy(false);
                }
              }}
            ...
            >
              {apiKeyBusy
                ? t("wizard.api_key.saving")
                : t("wizard.api_key.action")}
            </button>
          ) : (
...
function isClassifierCatalogModel(model: CatalogModel): boolean {
  return model.kind !== "ocr_bundle" && !model.kind.startsWith("ocr_");
}
