...
def build_parser() -> argparse.ArgumentParser:
    ...
    validate_parser.add_argument(
        "--config",
        help=(
            "Path to config YAML. Defaults to MEDIAREFINERY_CONFIG or "
    ...
    import_parser = config_subparsers.add_parser(
        "import-yaml",
        help="One-shot import of pipeline YAML into config.db (not used at runtime)",
    )
    import_parser.add_argument("yaml_path", help="Path to YAML file to import")
    import_parser.add_argument(
        "--data-dir",
        default="/data",
        help="Runtime data root containing databases/config.db",
    )
    import_parser.set_defaults(func=_cmd_config_import_yaml)
    ...
    report_parser.add_argument(
        "--format",
        help="Report format. Only markdown is implemented currently.",
    )
    report_parser.add_argument(
        "--output",
        help=(
            "Write to this path, or '-' for stdout. Defaults to "
            "reports.output_dir when a config is used."
        ),
    )
...
def _cmd_config_import_yaml(args: argparse.Namespace) -> int:
    from mediarefinery.settings.yaml_import import import_yaml_to_config_db

    count = import_yaml_to_config_db(
        Path(args.yaml_path), data_dir=Path(args.data_dir)
    )
    print(f"Imported {count} keys into {Path(args.data_dir) / 'databases' / 'config.db'}")
    return 0
...
def _cmd_report(args: argparse.Namespace) -> int:
    ...
    if report_format != MARKDOWN_FORMAT:
        print(
            "Report aborted: only markdown format is implemented currently",
            file=sys.stderr,
        )
        return 1
