...
``docs/reference/immich-api-compat.md``), encrypts the returned access token,
...
class AuthError(RuntimeError):
    """Raised when proxy login or revalidation fails for a non-credential reason.

    Wrong-password failures surface as :class:`InvalidCredentials` instead.
    """
...
def should_revalidate_session(
    ...
    *,
    ...
    """Return whether the session should be revalidated.
...
def persist_session(
    *,
    ...
    Bypasses :class:`UserScopedState` because the cookie has not been issued
