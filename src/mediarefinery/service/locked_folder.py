"""PIN-unlock and revert flow for the Immich Locked Folder.
...
- The reverse path requires a Bearer-session that has been
  PIN-unlocked via ``POST /api/auth/session/unlock``. The PIN is
  forwarded from the request straight to Immich
...
Per Immich 2.7.5 compatibility testing, the unlocked session is
...
class InvalidPin(UnlockError):
    """Represent InvalidPin."""

    pass


class UpstreamUnavailable(UnlockError):
    """Represent UpstreamUnavailable."""
...
def unlock_and_revert(
    *,
    ...
    """PIN-unlock the Immich session, revert assets, then re-lock.
