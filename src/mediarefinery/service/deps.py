...
def get_cipher(request: Request) -> AesGcmCipher:
    ...
    AesGcmCipher
    """
    return cast(AesGcmCipher, request.app.state.cipher)
...
def get_signer(request: Request) -> SessionCookieSigner:
    ...
    SessionCookieSigner
    """
    return cast(SessionCookieSigner, request.app.state.signer)
...
def get_immich_client(request: Request) -> httpx.Client:
    ...
    httpx.Client
    """
    return cast(httpx.Client, request.app.state.immich_client)


def client_ip(request: Request, config: ServiceConfig) -> str:
    """Resolve the client IP, honouring ``system.trusted_proxies`` from config.db.
...
def get_current_user(
    ...
    config : Annotated[ServiceConfig, Depends(get_service_config)]
    immich : Annotated[httpx.Client, Depends(get_immich_client)]
    session_cookie : Annotated[str | None, Cookie(alias=SESSION_COOKIE_NAME)], optional
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_401_UNAUTHORIZED, detail="invalid session expiry"
        ) from None
...
def require_csrf(
    ...
    Parameters
    ----------
    csrf_cookie : Annotated[str | None, Cookie(alias=CSRF_COOKIE_NAME)], optional
    csrf_header : Annotated[str | None, Header(alias=CSRF_HEADER_NAME)], optional
