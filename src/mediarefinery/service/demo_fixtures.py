...
1. **Auth.** An ``httpx.MockTransport`` returns canned responses for
   ...
   ``/auth/logout``, ``/users/me``, ``/server/version``,
   ``/server/about``). Anyone can sign in.
...
True``; production deployments use a real Immich connection.
"""
...
def synthetic_immich_handler(request: httpx.Request) -> httpx.Response:
    ...
    ``/server/version``, and ``/server/about`` return success.
    Everything else returns 404 so a misrouted call surfaces in tests
    ...
    if path == "/api/server/about":
        return httpx.Response(
            200, json={"version": "2.7.5", "demo": True}
        )
    return httpx.Response(404, json={"error": "demo fixture: path not stubbed"})


def build_demo_immich_client(*, base_url: str) -> httpx.Client:
    """Return an ``httpx.Client`` backed by the synthetic handler.

    Used by :func:`mediarefinery.service.app.create_app` when
...
def seed_demo_model(conn: sqlite3.Connection) -> None:
    ...
    so :func:`StateStore.active_model_sha256` returns the demo sha
