...
def _validate_video(data: Any, errors: list[str]) -> None:
    ...
    if max_frames is not None and (
        ...
        errors.append("video.max_frames: must be a positive integer")
    ...
    max_original_bytes = data.get("max_original_bytes")
    if max_original_bytes is not None and (
        not isinstance(max_original_bytes, int)
        or isinstance(max_original_bytes, bool)
        or max_original_bytes < 1
    ):
        errors.append("video.max_original_bytes: must be a positive integer")

    extraction_timeout_seconds = data.get("extraction_timeout_seconds")
    if extraction_timeout_seconds is not None and (
        not isinstance(extraction_timeout_seconds, int)
        or isinstance(extraction_timeout_seconds, bool)
        or extraction_timeout_seconds < 1
    ):
        errors.append("video.extraction_timeout_seconds: must be a positive integer")
        ...
        for media_type, rule in media_policy.items():
            ...
            for index, action in enumerate(on_match):
                ...
                if action == "archive":
                    ...
                    _warnings.warn(
                        f"{action_path}: 'archive' action is deprecated; "
