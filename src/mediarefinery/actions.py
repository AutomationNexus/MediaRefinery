"""Actions for MediaRefinery."""
from __future__ import annotations

from dataclasses import dataclass
from typing import Any

from .config import DESTRUCTIVE_ACTIONS, AppConfig
...
class ActionExecutionResult:
    """Represent ActionExecutionResult.
...
class ActionExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        config : AppConfig
        client : ImmichClient
        dry_run_override : bool | None, optional
        """
    ...
    def _add_to_review_album(
        ...
        assert plan.asset_id is not None  # caller verifies before dispatch
        ...
        try:
            if bool(self._actions.get("create_album_if_missing", True)):
                album_id: str = self._client.create_or_get_album(album_name)
            else:
                resolved = self._client.find_album_by_name(album_name)
                if resolved is None:
                    return self._failure(
                        plan,
                        action_name,
                ...
                album_id = resolved
            self._client.add_to_album(album_id, [asset_id])
        except Exception:
            return self._failure(
                plan,
                action_name,
    ...
    def _add_tag(
        ...
        try:
            if bool(self._actions.get("create_tag_if_missing", True)):
                tag_id: str = self._client.create_or_get_tag(tag_name)
            else:
                resolved_tag = self._client.find_tag_by_name(tag_name)
                ...
                tag_id = resolved_tag
            self._client.add_tag_to_asset(asset_id, tag_id)
        except Exception:
            return self._failure(
                plan,
                action_name,
    ...
    def _archive(
        ...
        assert plan.asset_id is not None  # caller verifies before dispatch
        # ``archive`` is superseded by ``move_to_locked_folder`` for
        # the service. The HTTP adapter does not implement
        # archive_asset, so the capability check below is the only
        # path live actions take. The action stays in validation for
        # existing configs; new configs should use
        # ``move_to_locked_folder`` instead.
