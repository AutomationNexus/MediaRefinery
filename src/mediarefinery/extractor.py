...
class MediaExtractionError(Exception):
    """Represent MediaExtractionError."""
...
class MediaExtractor:
    ...
    def video_frame_inputs(
        ...
        *,
        ...
        video_path : str | Path | None
        metadata : Mapping[str, str] | None, optional
        video_config : Mapping[str, object] | None, optional
        runtime_config : Mapping[str, object] | None, optional
        ...
        if media_type not in FRAME_EXTRACTABLE_MEDIA_TYPES:
            raise MediaExtractionError(
                asset_id=asset_id,
                media_type=media_type,
                source=DEFAULT_VIDEO_FRAME_SOURCE,
                message_code="unsupported_media_type",
                message="frame extractor only supports video and gif assets",
                details={"supported_media_types": sorted(FRAME_EXTRACTABLE_MEDIA_TYPES)},
            )
        ...
        frame_count = _positive_int(
            ...
            _positive_int(config.get("frame_count"), DEFAULT_VIDEO_FRAME_COUNT),
        )
        ...
        extraction_timeout_seconds = _positive_int(
            config.get("extraction_timeout_seconds"),
            DEFAULT_VIDEO_EXTRACTION_TIMEOUT_SECONDS,
        )
        frame_strategy = str(
...
def _extract_video_frame_paths(
    *,
    ...
    if duration_seconds is not None and duration_seconds > float(max_duration_seconds):
        raise MediaExtractionError(
            asset_id=asset_id,
            media_type=media_type,
            source=DEFAULT_VIDEO_FRAME_SOURCE,
            message_code="video_duration_exceeds_limit",
            message="media duration exceeds configured frame sampling limit",
            details={
                "max_duration_seconds": max_duration_seconds,
...
def _frame_classifier_input(
    *,
    ...
    return ClassifierInput(
        asset_id=asset_id,
        media_type="video" if asset_media_type == "video" else "image",
        metadata=input_metadata,
        data=frame_bytes,
        content_type=image_info.content_type,
        source=DEFAULT_VIDEO_FRAME_SOURCE,
