...
def test_image_extractor_returns_classifier_ready_input() -> None:
    """Test image extractor returns classifier ready input."""
...
def test_image_extractor_reports_structured_input_errors(
    ...
    """Test image extractor reports structured input errors."""
...
def test_image_extractor_rejects_non_image_assets_without_preview_download() -> None:
    """Test image extractor rejects non image assets without preview download."""
    with pytest.raises(MediaExtractionError) as exc_info:
        MediaExtractor().image_input(
            asset_id="video-1",
            media_type="video",
...
def test_extractor_does_not_log_image_bytes_on_errors(caplog) -> None:
    """Test extractor does not log image bytes on errors."""
...
def test_video_extractor_is_disabled_by_default(tmp_path) -> None:
    """Test video extractor is disabled by default."""
...
def test_video_extractor_deletes_temp_frames_on_success(
    ...
    """Test video extractor deletes temp frames on success."""
    video_path = tmp_path / "input.mp4"
    video_path.write_bytes(b"synthetic placeholder")
...
def test_video_extractor_honors_max_frames_and_timeout(
    tmp_path,
    monkeypatch,
) -> None:
    """Test video extractor honors max frames and timeout."""
    video_path = tmp_path / "input.mp4"
    video_path.write_bytes(b"synthetic placeholder")
    ...
    monkeypatch.setattr("mediarefinery.extractor.subprocess.run", fake_ffmpeg)
    ...
    ffmpeg_call = [call for call in calls if not _is_ffprobe(call["command"])][0]
    assert ffmpeg_call["timeout"] == 7


def test_video_extractor_refuses_duration_over_limit(
    tmp_path,
    monkeypatch,
) -> None:
    """Test video extractor refuses duration over limit."""
    video_path = tmp_path / "input.mp4"
    video_path.write_bytes(b"synthetic placeholder")
    ...
    assert exc_info.value.message_code == "video_duration_exceeds_limit"
...
def test_video_extractor_deletes_temp_frames_on_classifier_failure(
    ...
    """Test video extractor deletes temp frames on classifier failure."""
    video_path = tmp_path / "input.mp4"
    video_path.write_bytes(b"synthetic placeholder")
    temp_root = tmp_path / "frames"
    monkeypatch.setattr(
...
def test_video_extractor_deletes_temp_frames_on_ffmpeg_failure(
    ...
    """Test video extractor deletes temp frames on ffmpeg failure."""
    video_path = tmp_path / "input.mp4"
    video_path.write_bytes(b"synthetic placeholder")
    temp_root = tmp_path / "frames"
    monkeypatch.setattr(
...
def test_video_extractor_reads_generated_safe_video_when_ffmpeg_is_available(
    ...
    """Test video extractor reads generated safe video when ffmpeg is available."""
