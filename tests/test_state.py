...
def test_state_store_creates_schema_tables_deterministically(tmp_path) -> None:
    """Test state store creates schema tables deterministically."""
    path = tmp_path / "state.sqlite3"

    with StateStore(path) as store:
        assert store.schema_version() == SCHEMA_VERSION
        assert set(store.table_names()) == STATE_TABLES
        first_columns = {
...
def test_state_store_rejects_newer_schema_versions(tmp_path) -> None:
    """Test state store rejects newer schema versions."""
    path = tmp_path / "state.sqlite3"
...
def test_state_store_backfills_known_beta_columns(tmp_path) -> None:
    """Test state store backfills known beta columns."""
    path = tmp_path / "state.sqlite3"
    with sqlite3.connect(path) as conn:
        conn.executescript(
            """
...
def test_state_store_records_and_reads_sprint_004_metadata(tmp_path) -> None:
    """Test state store records and reads sprint 004 metadata."""
...
def test_state_record_classification_keeps_existing_action_hook(tmp_path) -> None:
    """Test state record classification keeps existing action hook."""
    asset = AssetRef("asset-1", "image", checksum="sha256:test")
...
def test_state_schema_has_no_blob_or_media_columns(tmp_path) -> None:
    """Test state schema has no blob or media columns."""
    with StateStore(tmp_path / "state.sqlite3") as store:
...
def test_state_needs_processing_hook_respects_reprocess_and_checksum(tmp_path) -> None:
    """Test state needs processing hook respects reprocess and checksum."""
    path = tmp_path / "state.sqlite3"
...
def test_asset_upsert_without_processing_remains_eligible(tmp_path) -> None:
    """Test asset upsert without processing remains eligible."""
    asset = AssetRef("asset-1", "image", checksum="sha256:test")
...
def test_error_records_redact_paths_secrets_and_media_like_values(tmp_path) -> None:
    """Test error records redact paths secrets and media like values."""
    path = tmp_path / "state.sqlite3"
    asset = AssetRef("asset-1", "image", checksum="sha256:test")
    long_base64 = "A" * 100
...
def test_config_and_model_metadata_do_not_store_private_paths(tmp_path) -> None:
    """Test config and model metadata do not store private paths."""
    path = tmp_path / "state.sqlite3"
