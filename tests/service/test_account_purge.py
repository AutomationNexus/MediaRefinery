"""DELETE /api/me — account purge (threat-model T20).
...
def _user_rows(conn, table: str, user_id: str) -> int:
    return _row_count(
        conn, f"SELECT COUNT(*) FROM {table} WHERE user_id = ?", (user_id,)
    )


def test_delete_me_purges_all_user_rows(app):
    """Test delete me purges all user rows."""
    with TestClient(app) as client:
        ...
        assert _user_rows(conn, "users", "user-alice") == 1
        assert _user_rows(conn, "sessions", "user-alice") >= 1
        assert _user_rows(conn, "user_api_keys", "user-alice") >= 1
        assert _user_rows(conn, "user_config", "user-alice") >= 1
        pre_audit = _user_rows(conn, "audit_log", "user-alice")
        assert pre_audit >= 1

        r = client.delete(f"{API_PREFIX}/me", headers=h)
        assert r.status_code == 204

        # Cookies cleared on the response.
        assert (
            SESSION_COOKIE_NAME not in r.cookies
            or r.cookies.get(SESSION_COOKIE_NAME) in (None, "")
        )
        ...
        for table in (
            ...
            assert _user_rows(conn, table, "user-alice") == 0, (
                f"{table} still has rows for purged user"
            )

        # Audit_log entries are anonymized to the sentinel.
        assert _user_rows(conn, "audit_log", "user-alice") == 0
        anon = _user_rows(conn, "audit_log", "user_deleted")
        assert anon >= pre_audit


def test_delete_me_does_not_touch_other_users(app):
    """Test delete me does not touch other users."""
    with TestClient(app) as client:
        ...
        r = client.delete(f"{API_PREFIX}/me", headers=a_h)
        assert r.status_code == 204

        # Bob's data is intact.
        store = app.state.store
        conn = store._conn
        assert _user_rows(conn, "users", "user-bob") == 1
        assert _user_rows(conn, "user_config", "user-bob") == 1
        assert _user_rows(conn, "user_api_keys", "user-bob") == 1
        assert _user_rows(conn, "audit_log", "user-bob") >= 1
