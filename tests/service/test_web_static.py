"""Dashboard static-serve and CSP header tests.
...
``npm run build``; here we pass a stub ``web_root`` into
``create_app`` so the tests do not depend on the node toolchain.
"""
...
def _make_app(tmp_path, monkeypatch, *, with_bundle: bool):
    ...
    app = create_app(config=config, web_root=web_root)
...
def test_security_headers_on_api_response(tmp_path, monkeypatch):
    """Test security headers on api response."""
...
def test_static_index_served_when_bundle_present(tmp_path, monkeypatch):
    """Test static index served when bundle present."""
...
def test_root_404_when_bundle_missing(tmp_path, monkeypatch):
    """Test root 404 when bundle missing."""
    client = _make_app(tmp_path, monkeypatch, with_bundle=False)
    resp = client.get("/")
