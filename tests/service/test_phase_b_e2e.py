...
def test_phase_b_exit_criterion_single_user(app):
    """Test phase b exit criterion single user."""
    with TestClient(app) as client:
        ...
        # Undo it.
        r = client.post(f"{API_PREFIX}/scans/{run_id}/undo", headers=h)
        assert r.status_code == 200
...
def test_phase_b_exit_criterion_multi_tenant_isolation(app):
    ...
    with TestClient(app) as client:
        ...
        run_id_alice = client.post(
            f"{API_PREFIX}/scans", headers=a_h
        ).json()["run_id"]
        ...
        bob_cats = client.get(f"{API_PREFIX}/me/categories").json()
        assert bob_cats["categories"] == {}
        assert client.get(f"{API_PREFIX}/me/api-key").json()["api_keys"] == []
        assert client.get(f"{API_PREFIX}/scans").json()["scans"] == []

        b_audit = client.get(f"{API_PREFIX}/audit").json()["entries"]
        # Bob's audit only contains his own login.
        assert {entry["action"] for entry in b_audit} <= {"login"}

        r = client.get(f"{API_PREFIX}/scans/{run_id_alice}")
        assert r.status_code == 404
        assert "alice" not in r.text.lower()

        r = client.post(
            f"{API_PREFIX}/scans/{run_id_alice}/undo", headers=b_h
        )
        assert r.status_code == 404
        ...
        a_categories = client.get(f"{API_PREFIX}/me/categories").json()
...
def test_concurrency_cap_returns_409(app, monkeypatch):
    ...
    # the concurrency cap.
    """Test concurrency cap returns 409."""
    import threading
    ...
    with TestClient(app) as client:
        csrf = _login(client, "alice@x.invalid", ALICE_PW)
        h = {"X-CSRF-Token": csrf}
        r1 = client.post(f"{API_PREFIX}/scans", headers=h)
        assert r1.status_code == 202
        started.wait(timeout=2.0)
        r2 = client.post(f"{API_PREFIX}/scans", headers=h)
...
def test_csrf_required_for_mutations(app):
    ...
    with TestClient(app) as client:
        ...
        r = client.post(f"{API_PREFIX}/scans")
        assert r.status_code == 403
...
def test_unauthenticated_endpoints_return_401(app):
    """Test unauthenticated endpoints return 401."""
    with TestClient(app) as client:
...
def test_openapi_lists_api_endpoints(app):
    """Test openapi lists api endpoints."""
    with TestClient(app) as client:
        r = client.get(f"{API_PREFIX}/openapi.json")
        assert r.status_code == 200
        paths = r.json()["paths"]
        for required in (
            "/api/auth/login",
...
def test_privacy_no_secrets_after_full_session(app):
    """Test privacy no secrets after full session."""
    with TestClient(app) as client:
        csrf = _login(client, "alice@x.invalid", ALICE_PW)
        h = {"X-CSRF-Token": csrf}
        client.post(
        ...
        run_id = client.post(f"{API_PREFIX}/scans", headers=h).json()["run_id"]
        _wait_for_scan(client, run_id)
        for path in (
            "/me", "/me/categories", "/me/policies", "/me/api-key",
            "/scans", f"/scans/{run_id}", "/audit",
        ):
            text = client.get(f"{API_PREFIX}{path}").text
