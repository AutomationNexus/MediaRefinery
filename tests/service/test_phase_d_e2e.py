"""Real-pipeline service end-to-end test.
...
# Imports follow importorskip on purpose; modules below transitively
# pull in fastapi/httpx and would error at collection on slim envs.
# ruff: noqa: E402
...
def _seed_active_model(store) -> str:
    ...
    store._conn.execute(
        ...
        "VALUES (?,?,?,?,1)",
        ("adamcodd-vit-nsfw-quantized", "catalog", sha, "Apache-2.0"),
    )
    store._conn.commit()
    return sha
...
def test_phase_d_exit_criterion(context):
    ...
    with TestClient(app) as client:
        ...
        # share the mock Immich client with the unlock proxy. The UI
        ...
        # Unlock cycle.
        r = client.post(
            f"{API_PREFIX}/me/locked-folder/unlock",
        ...
        audit = client.get(f"{API_PREFIX}/audit").json()
...
def test_phase_d_two_user_isolation(context):
    ...
    with TestClient(app) as client:
        ...
        scans = client.get(f"{API_PREFIX}/scans").json()["scans"]
        assert scans == []
        audit_entries = client.get(f"{API_PREFIX}/audit").json()["entries"]
        assert all(
