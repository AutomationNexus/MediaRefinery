"""ONNX classifier session cache tests."""
...
def test_profile_from_catalog_entry_pulls_preprocessing(tmp_path):
    """Test profile from catalog entry pulls preprocessing."""
...
def test_profile_from_adult_subtype_model_uses_registered_metadata(tmp_path):
    """Test profile from adult subtype model uses registered metadata."""
    model_path = tmp_path / "subtype.onnx"
    model_path.write_bytes(b"fake")
    profile = profile_from_adult_subtype_model(
        {
            "version": "local-subtypes",
            "sha256": "c" * 64,
            "metadata": {
    ...
    assert profile.output_mapping == {
        "custom_one": "custom_one",
        "custom_two": "custom_two",
    }
    assert profile.input_size == 384
...
def test_cache_warm_reload_on_model_swap(tmp_path):
    """Test cache warm reload on model swap."""
...
def test_cache_get_none_returns_none(tmp_path):
    """Test cache get none returns none."""
    cache = ClassifierSessionCache(
        models_dir=tmp_path,
        catalog=[],
        backend_factory=_CountingBackend,
...
def test_factory_adapter_matches_runner_seam(tmp_path):
    """Test factory adapter matches runner seam."""
    entry = _entry("model-a", "a" * 64)
    cache = ClassifierSessionCache(
        models_dir=tmp_path,
        catalog=[entry],
