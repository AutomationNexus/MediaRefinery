"""Bootstrap endpoint, first-user-admin, and reclassify CTA tests."""
...
def test_bootstrap_status_initial(app):
    """Test bootstrap status initial."""
    with TestClient(app) as client:
        r = client.get(f"{API_PREFIX}/setup/bootstrap")
        body = r.json()
        assert body == {
...
def test_bootstrap_accept_terms(app):
    """Test bootstrap accept terms."""
    with TestClient(app) as client:
        r = client.post(
            f"{API_PREFIX}/setup/bootstrap",
...
def test_bootstrap_refuses_without_acceptance(app):
    """Test bootstrap refuses without acceptance."""
    with TestClient(app) as client:
        r = client.post(
            f"{API_PREFIX}/setup/bootstrap",
...
def test_first_user_promoted_to_admin_even_if_not_immich_admin(app):
    """Test first user promoted to admin even if not immich admin."""
    with TestClient(app) as client:
...
def test_second_non_admin_user_is_not_admin(app):
    """Test second non admin user is not admin."""
    with TestClient(app) as client:
...
def test_immich_admin_remains_admin(app):
    """Test immich admin remains admin."""
    with TestClient(app) as client:
...
def test_reclassify_cta_appears_after_active_model_changes(app):
    """Test reclassify cta appears after active model changes."""
    with TestClient(app) as client:
        csrf = _login(client, "alice@x.invalid")
        h = {"X-CSRF-Token": csrf}

        # Initial state: no active model, no last_seen → no CTA.
        body = client.get(f"{API_PREFIX}/me/categories").json()
...
def test_admin_count_starts_at_zero():
    """Test admin count starts at zero."""
    from mediarefinery.service.state_store import StateStore
