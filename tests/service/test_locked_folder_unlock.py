"""PIN-unlock and revert flow privacy tests.
...
def test_unlock_pin_never_appears_in_response_or_audit(app_state):
    """Test unlock pin never appears in response or audit."""
    app, _ = app_state
    with TestClient(app) as client:
        csrf = _login(client)
        run_id = _seed_locked_run(app)
...
def test_unlock_invalid_pin_returns_401(tmp_path, monkeypatch):
    """Test unlock invalid pin returns 401."""
...
def test_unlock_no_locked_actions_returns_400(app_state):
    """Test unlock no locked actions returns 400."""
    app, _ = app_state
    with TestClient(app) as client:
        csrf = _login(client)
...
def test_unlock_unknown_run_returns_404(app_state):
    """Test unlock unknown run returns 404."""
    app, _ = app_state
    with TestClient(app) as client:
        csrf = _login(client)
        r = client.post(
